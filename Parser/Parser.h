...
/*
  ...
  Maintenance History:
  ====================
  ver 1.5: 12 March, 2016
  - No changes made, simply added more references
  ver 1.4 : 15 Feb 16
...
protected:
  std::vector<IAction*> actions;                      //Each rules have a list of actions to perform
};
...
public:
  ...
  bool parse();
  bool next();
  void updateFileName(std::string fileSpec);
  std::string getFileName();
private:
  ITokCollection* pTokColl;
  std::vector<IRule*> rules;               //Contains Rules defined in ActionsAndRules.h + .cpp
  std::string fileName;
};
